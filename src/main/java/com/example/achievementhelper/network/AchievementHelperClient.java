...
public class AchievementHelperClient {
    ...
    public static void handleAchievementData(List<AchievementDataPacket.AchievementData> achievements) {
        Minecraft.getInstance().execute(() -> {
            if (Minecraft.getInstance().screen instanceof AchievementListScreen screen) {
                screen.updateAchievements(achievements);
            }
        });
    }
}